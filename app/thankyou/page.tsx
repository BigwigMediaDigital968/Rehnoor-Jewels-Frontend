...
// GA Tag version of Thank You page with GA4 purchase event tracking
...
// 1. Updated OrderData interface to include item details for GA4
interface OrderItem {
  productId: string;
  name?: string;
  price?: number;
...
interface OrderData {
  ...
  total: number;
  paymentMethod: string;
  items?: OrderItem[]; // Add items array here
}
...
export default function ThankYouPage() {
  ...
  // Ref guard to prevent double-firing during React 18 strict mode double-renders
  ...
  // 2. Trigger GA4 Purchase Event once order is set
  useEffect(() => {
