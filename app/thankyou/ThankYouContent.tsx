...
//
// Client component handling the Thank You state, storage clearance, and analytics.
...
export default function ThankYouContent() {
  ...
  useEffect(() => {
    try {
      // 1. Wipe Zustand Stores (Cart, Buy-Now state, Coupons, Checkout Forms)
      useCartStore.getState().clearCart();
      useCheckoutStore.getState().reset();

      // 2. Read stored order or URL params
      const raw = sessionStorage.getItem("rj_last_order");
      const orderIdParam =
        searchParams.get("order_id") || searchParams.get("orderId");

      if (raw) {
        const parsedOrder: OrderData = JSON.parse(raw);
        if (orderIdParam) parsedOrder.orderNumber = orderIdParam;
        setOrder(parsedOrder);
        sessionStorage.removeItem("rj_last_order");
      } else if (orderIdParam) {
        setOrder({
          orderNumber: orderIdParam,
          total: Number(searchParams.get("amount") || 0),
          paymentMethod: searchParams.get("payment_method") || "prepaid",
          items: [],
        });
      }
    } catch (e) {
      console.error("Error initializing thank you page state:", e);
    }
    setLoaded(true);
  }, [searchParams]);
      ...
      window.dataLayer.push({
        ...
        ecommerce: {
          ...
          items: (order.items || []).map((i) => ({
            item_id: i.productId,
            item_name: i.name || "Jewellery Item",
            item_variant: i.variant?.variantId || i.variant?.name,
            price: i.price,
            quantity: i.qty,
          })),
        },
