...
//     return available.length === 1 ? available[0].label : null;
//   });
...
//   const handleAdd = () => {
//     if (!selectedSize) {
//       // Flash the size picker to prompt selection
//       setSizeError(true);
...
//               [data-qv="panel"] { margin: 1rem; max-height: 88vh; }
//             }
//           `}</style>
...
//                   <span
//                     className="font-cinzel text-[8px] font-bold tracking-widest px-2.5 py-1 bg-amber-200 rounded-full shadow-md"
...
//                 <div className="mb-5">
//                   <p
//                     className="font-cinzel text-[10px] tracking-widest uppercase font-bold mb-2.5 flex items-center gap-2"
...
//                     >
//                       {selectedSize ||
//                         (sizeError ? "Please select a size" : "Select")}
//                     </span>
//                   </p>
//                   <motion.div
...
export default function QuickViewModal({
  ...
  // Initialize selected size using normalized schema array
  const [selectedSize, setSelectedSize] = useState<string | null>(() => {
    const available = normalizedSizes.filter((s) => s.available);
    return available.length === 1 ? available[0].label : null;
  });
  ...
  const handleAdd = () => {
    ...
    const variantSnapshot = {
      variantId:
        ...
        selectedSize.toLowerCase().replace(/\s+/g, "-"),
      title: selectedSize,
      options: matchedVariant?.options || { Size: selectedSize },
      price: matchedVariant?.price ?? priceNum,
      originalPrice: matchedVariant?.originalPrice ?? originalPriceNum,
      image: matchedVariant?.images?.[0]?.src ?? product.images[0]?.src ?? "",
    };
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          <style>{`
            @media (min-width: 640px) {
              [data-qv="panel"] { margin: 1rem; max-height: 88vh; overflow: hidden; }
            }
          `}</style>
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-2">
            ...
            >
              ...
              {/* Tag Label */}
              {product.tag && (
                <div className="absolute top-4 right-14 z-10 pointer-events-none">
                  <span
            ...
            >
              ...
              {/* Dynamically Rendered Size Picker */}
              {normalizedSizes.length > 1 && (
                <div className="mb-5">
                  ...
                  >
                    ...
                    >
                      {selectedSize ||
                        (sizeError ? "Please select an option" : "Select")}
                    </span>
                  </p>
                  <motion.div
                    animate={sizeError ? { x: [0, -6, 6, -4, 4, 0] } : {}}
                  ...
                  >
                    {normalizedSizes.map((s) => (
                      <button
                        ...
                        aria-label={`Option ${s.label}${!s.available ? " — unavailable" : ""}`}
                      >
                        {s.label}
                        {!s.available && (
                          <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                            <div
                              className="w-4/5 h-px rotate-12"
                              style={{ background: "var(--rj-bone)" }}
                            />
                          </div>
                        )}
              ...
              <Link
                ...
                className="flex items-center justify-center py-3 font-cinzel text-[10px] tracking-widest uppercase font-bold rounded-full transition-all duration-300 hover:opacity-90 active:scale-95 mb-4"
                style={{
                  background: "var(--gradient-gold, var(--rj-gold))",
                  color: "var(--rj-charcoal)",
                  boxShadow: "0 4px 16px rgba(252,193,81,0.25)",
                  cursor: "pointer",
                  textAlign: "center",
                  textDecoration: "none",
                }}
