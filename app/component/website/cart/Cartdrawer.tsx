...
function CouponSection() {
  ...
  const handleApply = async () => {
    ...
    setLoading(true);
    setError("");
    const result = await validateCoupon(code.trim(), subtotal(), items);
    setLoading(false);
  ...
  if (coupon?.code) {
    return (
      <div>
        ...
        {error && (
          <p
            className="font-cinzel text-[8px] mt-1.5 pl-1"
            style={{ color: "#ef4444" }}
          >
            {error}
          </p>
...
//             🎉 You&rsquo;re saving{" "}
//             <strong className="text-[#FCC131] font-bold">
//               {fmtPrice(save)}
//             </strong>
//             !
//           </span>
...
//               style={{
//                 color:
//                   shipping === 0 ? "var(--rj-emerald)" : "var(--rj-charcoal)",
//               }}
//             >
...
function DrawerSummary({ onClose }: { onClose: () => void }) {
  ...
  // Format cart items for Shiprocket Access Token API format
  const shiprocketFormattedItems = activeItems.map((item) => ({
    variantId:
      (item.variant as { _id?: string; id?: string })?._id ||
      (item.variant as { _id?: string; id?: string })?.id ||
      item.id,
    id: item.id,
    quantity: item.qty,
  }));
        ...
        <>
          ...
          <motion.div
            ...
            transition={{
              ...
              ease: [0.215, 0.61, 0.355, 1.0], // Custom cubic-bezier (easeOutCubic) for ultra-smooth easing
            }}
            onClick={closeDrawer}
