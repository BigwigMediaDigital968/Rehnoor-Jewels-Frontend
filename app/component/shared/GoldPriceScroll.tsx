...
interface MetalApiResponse {
  ...
  timestamp: number;
  rates: {
    // API returns values mapped as USDXAU or native codes depending on internal provider configurations
    XAU?: number;
    INR?: number;
    USDXAU?: number;
    USDINR?: number;
...
function computeGoldData(rates: MetalApiResponse["rates"]): GoldData | null {
  // Normalize key variants returned by API (e.g., XAU vs USDXAU)
  const xauUsdRate = rates.USDXAU ?? rates.XAU;
  const inrUsdRate = rates.USDINR ?? rates.INR;

  if (!xauUsdRate || !inrUsdRate) return null;

  // If the API returns values as inversion units (e.g., 1 USD = 0.0004 XAU)
  const isXauInverted = xauUsdRate < 1;
  const isInrDirect = inrUsdRate > 1;

  const goldPriceUSDPerOz = isXauInverted ? 1 / xauUsdRate : xauUsdRate;
  const usdToInrExchange = isInrDirect ? inrUsdRate : 1 / inrUsdRate;

  // Convert ounce to single gram target value in Indian Rupees
  const goldPricePerOzINR = goldPriceUSDPerOz * usdToInrExchange;
  const g24 = goldPricePerOzINR / TROY_OZ_TO_GRAM;
...
export default function GoldMarqueeScroll() {
  const [data, setData] = useState<GoldData | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [paused, setPaused] = useState(false);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const json: MetalApiResponse = await response.json();
      if (!json.success) throw new Error("API validation failure token");

      const computed = computeGoldData(json.rates);
      if (!computed) throw new Error("Faulty computation mappings");

      setData(computed);
      setError(null);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : "Unknown connectivity issue");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Cinzel:wght@600;700&family=Cormorant+Garamond:wght@400;500;600&family=DM+Sans:wght@400;500&display=swap');
        ...
        .gm-wrapper {
          ...
          overflow: hidden;
          position: relative;
          background: linear-gradient(180deg, #002618 0%, #001a10 100%);
          border-top: 1px solid rgba(252,193,81,0.1);
          border-bottom: 1px solid rgba(252,193,81,0.1);
          height: 48px;
          display: flex;
          align-items: center;
        }

        /* Responsive Side Gradient Overlays */
        .gm-wrapper::before, .gm-wrapper::after {
          content: ''; position: absolute; top: 0; bottom: 0; width: 40px; z-index: 5; pointer-events: none;
        }
        @media (min-width: 640px) {
          .gm-wrapper::before, .gm-wrapper::after { width: 80px; }
        }
        .gm-wrapper::before { left: 0; background: linear-gradient(to right, #002618, transparent); }
        .gm-wrapper::after { right: 0; background: linear-gradient(to left, #002618, transparent); }
        ...
        .gm-chip {
          ...
          gap: 0.45rem;
          padding: 0 1.25rem;
          border-right: 1px solid rgba(252,193,81,0.08);
        }
        @media (min-width: 640px) {
          .gm-chip { gap: 0.6rem; padding: 0 1.8rem; }
        }
        ...
        .gm-chip.highlighted .gm-label { color: #fcc151; }
        ...
        @media (min-width: 640px) { .gm-value { font-size: 1.25rem; } }
        
        .gm-chip.highlighted .gm-value {
          background: linear-gradient(135deg, #fcc151 0%, #f5a623 100%);
        ...
        .gm-dot {
          width: 3px; height: 3px; border-radius: 50%; background: rgba(252,193,81,0.25); margin-left: 0.4rem;
        }

        .gm-status {
          padding: 0 1.5rem; color: rgba(255,255,255,0.4); font-size: 0.72rem; font-family: var(--font-body), sans-serif;
        }
        .gm-status.error { color: #f87171; }

        .gm-skeleton {
          width: 130px; height: 14px; background: rgba(255,255,255,0.04); border-radius: 3px; position: relative; overflow: hidden; margin: 0 1.2rem;
        }
        .gm-skeleton::after {
          content: ''; position: absolute; inset: 0; background: linear-gradient(90deg, transparent, rgba(255,255,255,0.06), transparent);
          transform: translateX(-100%); animation: gm-shimmer 1.4s infinite;
        }
        @keyframes gm-shimmer { 100% { transform: translateX(100%); } }
      `}</style>

      <div
        className="gm-wrapper"
        ...
        {loading ? (
          <div style={{ display: "flex", alignItems: "center" }}>
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="gm-skeleton" />
            ))}
          </div>
        ) : error ? (
          <div className="gm-status error">
            ⚠️ Live Gold Rates Temporarily Unavailable ({error})
          </div>
        ) : (
          <div className={`gm-track ${paused ? "paused" : ""}`}>
            {repeatedItems.map((item, i) => (
              <div
                key={`${item.id}-${i}`}
                className={`gm-chip ${item.highlight ? "highlighted" : ""}`}
              >
                <span className="gm-label">{item.label}</span>
                <span className="gm-value">{item.value}</span>
                <span className="gm-sub">{item.sub}</span>
